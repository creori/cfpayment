...
$Id: _process.cfm 75 2008-11-18 19:18:36Z markmazelin $
--->
...
<cfcatch>
	...
	<cfset gwParams.Path = "authorizenet.authorizenet" />
	<!--- these following params aren't needed for the bogus gateway, but should normally be filled in --->
	<cfset gwParams.MerchantAccount = "6ay34R37ftsRGF24" /><!---Creori, test acct.--->
	<cfset gwParams.userName = "62weFnt8Squs" /><!---Creori, test acct.--->
	<cfset gwParams.TestMode = "false" />
</cfcatch>
</cftry>
...
<cftry>
	...
	<cfparam name="form.IPAddress" default="#cgi.REMOTE_ADDR#">
	...
	<cfparam name="form.TransactionType" default="AUTH">
	...
	<cfif structKeyExists(form, "submitBtn")>
		...
		<cftry>
			...
			<cfif not ArrayLen(errors)>
				...
				<cfset options.IPAddress = form.IPAddress />
				...
				<cfset authResponse=gw.purchase(money, creditCard, options)>
				...
				<cfelse>
					...
					<cfset ArrayAppend(errors, "authResponse:message-" & authResponse.getMessage())>
				</cfif>
			</cfif>
		...
		<cfcatch type="cfpayment">
			...
			<cfset ArrayAppend(errors, "cfcatch:cfpayment-" & cfcatch.message)>
		</cfcatch>
		<cfcatch>
			<!--- <cfdump var="#cfcatch#"><cfabort> --->
			<cfset ArrayAppend(errors, "cfcatch:any-" & cfcatch.message)>
		</cfcatch>
		</cftry>
	</cfif>
