<cfcomponent displayname="Gateway" extends="cfpayment.api.gateway.base" output="false" hint="Bogus gateway demonstrates an implementation">
	...
	<cfset variables.cfpayment.SuccessMessage = "Bogus Gateway: Forced success; use CC number 1 or 4111111111111111 for success, 2 for decline, anything else for error (#expandPath(".")#)" />
	<cfset variables.cfpayment.DeclineMessage = "Bogus Gateway: Forced failure; use CC number 1 or 4111111111111111 for success, 2 for decline, anything else for error (#expandPath(".")#)" />
	<cfset variables.cfpayment.ErrorMessage = "Bogus Gateway: Forced Error; use CC number 1 for success, 2 for decline, anything else for error (#expandPath(".")#)" />
	...
	<cffunction name="authorize" output="false" access="public" returntype="any" hint="Verifies payment details with merchant bank">
		...
		<!--- Just for demonstration purposes; pass CC# 1 or 4111111111111111for sucecss, CC# 2 for failure, anything else for exception --->
		<cfswitch expression="#arguments.account.getAccount()#">
			<cfcase value="1,4111111111111111" delimiters=",">
				<cfset response.setMessage(variables.cfpayment.SuccessMessage) />
				<cfset response.setAuthorization(123456) />
				<cfset response.setTransactionID(getTickCount()) />
				<cfset response.setStatus(getService().getStatusSuccessful()) />
				<cfset structInsert(results, "result", "APPROVED", "yes") />
			...
			<cfcase value="2" delimiters=",">
				...
				<cfset response.setTransactionID(getTickCount()) />
				<cfset structInsert(results, "result", "NOT APPROVED", "yes") />
				<cfset response.setParsedResult(results) />
			</cfcase>
			<cfdefaultcase>
				...
				<cfset response.setTransactionID(getTickCount()) />
				<cfset structInsert(results, "result", "ERROR", "yes") />
				<cfset response.setParsedResult(results) />
			</cfdefaultcase>
		</cfswitch>
	...
	<cffunction name="capture" output="false" access="public" returntype="any" hint="Confirms an authorization with direction to charge the account">
		...
		<cfswitch expression="#len(arguments.authorization) GT 0#">
			<cfcase value="true">
				<cfset response.setMessage(variables.cfpayment.SuccessMessage) />
				<cfset response.setAuthorization(123456) />
				<cfset response.setStatus(getService().getStatusSuccessful()) />
				<cfset response.setTransactionID(getTickCount()) />
				<cfset structInsert(results, "result", "CAPTURED", "yes") />
				<cfset structInsert(results, "reference", "123456", "yes") />
				<cfset structInsert(results, "additional", "Bogus Gateway", "yes") />
				<cfset response.setParsedResult(results) />
			</cfcase>
			<cfdefaultcase>
				<cfset response.setStatus(getService().getStatusDeclined()) />
				<cfset response.setMessage(variables.cfpayment.DeclineMessage) />
				<cfset response.setTransactionID(getTickCount()) />
				<cfset structInsert(results, "result", "NOT CAPTURED", "yes") />
				<cfset structInsert(results, "reference", "123456", "yes") />
				<cfset structInsert(results, "additional", "Bogus Gateway", "yes") />
				<cfset response.setParsedResult(results) />
			</cfdefaultcase>
