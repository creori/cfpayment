...
<cfcomponent displayname="Authorize.net AIM Interface" extends="cfpayment.api.gateway.base" hint="Authorize.net AIM Gateway" output="false">
	...
	<cffunction name="process" output="false" access="private" returntype="any">
		...
		<cfscript>
			...
			if (structKeyExists(requestData, "x_card_code"))
				requestData.x_card_code = "...";
			if (structKeyExists(requestData, "card"))
				requestData.card = "..." & right(requestData.card, 4);
			if (structKeyExists(requestData, "x_card_num"))
				requestData.x_card_num = "..." & right(requestData.x_card_num, 4);
