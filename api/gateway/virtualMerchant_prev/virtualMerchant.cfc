...
<cfcomponent displayname="VirtualMerchant Interface" extends="cfpayment.api.gateway.base" hint="VirtualMerchant Gateway" output="false">
	...
	<cfscript>
		...
		variables.cfpayment.GATEWAY_TEST_URL = "https://demo.myvirtualmerchant.com/VirtualMerchantDemo/login.do";
	...
	<cffunction name="process" output="false" access="private" returntype="any">
		...
		<cfscript>
			...
			structInsert(p, "ssl_customer_code", "1111", "yes");
			structInsert(p, "ssl_receipt_link_method", "POST", "yes");
			structInsert(p, "ssl_cvv2cvc2_indicator", "1", "yes");
			...
			structInsert(p, "ssl_test_mode", "FALSE", "yes"); // Maybe: getTestMode()
			...
			if (NOT response.hasError()) {
				...
				if (len(response.getResult())) {
					...
					if (structKeyExists(results, "ssl_cvv2cvc2_resp") AND results.ssl_cvv2cvc2_resp NEQ "P")
						response.setCVVCode(results.ssl_cvv2cvc2_resp);					
			...
			if (structKeyExists(requestData, "expYear"))
				requestData.expYear = "...";
			if (structKeyExists(requestData, "x_login"))
				requestData.x_login = left(requestData.x_login, 3) & "...";
			if (structKeyExists(requestData, "x_tran_key"))
				requestData.x_tran_key = left(requestData.x_tran_key, 3) & "...";
	...
	<cffunction name="addCustomer" output="false" access="private" returntype="any" hint="Add customer contact details to the request object">
		...
		<cfscript>
			...
			if (structKeyExists(arguments.options, "address") AND structKeyExists(arguments.options.address, "phone"))
				structInsert(arguments.post, "ssl_phone", options.address.phone); // Customer’s phone number
			else
				structInsert(arguments.post, "ssl_phone", ""); // No phone number

			if (structKeyExists(arguments.options, "email"))
				structInsert(arguments.post, "ssl_email", arguments.options.email); // Customer’s e-mail address
			else
				structInsert(arguments.post, "ssl_email", ""); // No email
	...
	<cfscript>
		...
		function parseResponse(gatewayResponse) {
			...
			insertResult(results, response, "11", "Method", "x_method", "");
			insertResult(results, response, "12", "Transaction Type", "x_type", "");
			insertResult(results, response, "13", "Customer ID", "x_cust_id", "");
			insertResult(results, response, "14", "Cardholder First Name", "x_first_name", "");
			insertResult(results, response, "15", "Cardholder Last Name", "x_last_name", "");
			insertResult(results, response, "16", "Company", "x_company", "");
			insertResult(results, response, "17", "Billing Address", "x_address", "");
			insertResult(results, response, "18", "City", "x_city", "");
			insertResult(results, response, "19", "State", "x_state", "");
			insertResult(results, response, "20", "ZIP", "x_zip", "");
			insertResult(results, response, "21", "Country", "x_country", "");
			insertResult(results, response, "22", "Phone", "x_phone", "");
			insertResult(results, response, "23", "Fax", "x_fax", "");
			insertResult(results, response, "24", "E-Mail", "x_email", "");
